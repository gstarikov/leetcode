...
func TestFindWords(t *testing.T) {
	tests := []struct {
		...
		{ //2
			...
			res:   []string{"eaabcdgfa", "eaafgdcba"},
		},
		{ //3
			board: [][]byte{
				{'o', 'a', 'b', 'n'},
		...
		{ //4
			...
			res:   []string{"abcdefg", "befa", "eaabcdgfa", "gfedcbaaa"},
		},
		{ //5
			board: [][]byte{
				{'b', 'a', 'b', 'a', 'b', 'a', 'b', 'a', 'b', 'a'},
			...
			res:   []string{"ababababab"},
		},
	}

	for i, test := range tests {
		res := FindWords(test.board, test.words)
		sort.Strings(res)
...
func TestPrefixTree2(t *testing.T) {
	tests := []struct {
		...
		{
			op:   []string{"Insert", "Search", "Search", "Search", "StartsWith", "StartsWith", "StartsWith"},
			data: []string{"hello", "hell", "helloa", "hello", "hell", "helloa", "hello"},
			res:  []bool{false, false, false, true, true, false, true},
		},
		{
			op:   []string{"Insert", "Search", "Search", "StartsWith", "StartsWith", "Insert", "Search", "StartsWith", "Insert", "Search", "StartsWith"},
			data: []string{"ab", "abc", "ab", "abc", "ab", "ab", "abc", "abc", "abc", "abc", "abc"},
			res:  []bool{false, false, true, false, true, false, false, false, false, true, true},
		},
		{
			op:   []string{"Insert", "Search", "Search", "Insert", "Search", "Search", "Remove", "Search", "Search"},
			data: []string{"aaa", "aaa", "aa", "aa", "aaa", "aa", "aaa", "aaa", "aa"},
			res:  []bool{false, true, false, true, true, true, true, false, true},
		},
	}
	...
	for i, test := range tests {
		...
		for j := range test.op {
			...
			data := test.data[j]
			exp := test.res[j]
			res := reflect.ValueOf(&tr).MethodByName(op).Call([]reflect.Value{reflect.ValueOf(data)})
			if len(res) > 0 {
				rr := res[0].Bool()
				assert.Equalf(t, exp, rr, "case(%d->%d)", i, j)
			}
		}
	}
}
